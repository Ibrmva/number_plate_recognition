...
from inference_parseq import parseq_read_license_plate
from config import *
...
def annotate_and_log(cam_id, frame_nmr, frame, track_ids, lp_dets):
    ...
    for i, lp in enumerate(lp_dets_list):
        ...
        text, text_score = parseq_read_license_plate(lp_crop)
        if text:
...
def display_loop(num_cams):
    ...
    try:
        ...
        logging.info("KeyboardInterrupt detected in display loop")
        shutdown_event.set()
    finally:
...
def run_realtime():
    ...
    try:
        ...
        logging.info("KeyboardInterrupt detected in main thread")
        shutdown_event.set()
